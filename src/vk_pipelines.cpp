...
bool vkutil::load_shader_module(const char* filePath, VkDevice device, VkShaderModule* outShaderModule)
{
    ...
    assert(fileSize % sizeof(uint32_t) == 0 && "expect fileSize a multiple of sizeof(uint32_t)");
...
VkPipeline PipelineBuilder::build_pipeline(VkDevice device)
{
    ...
    // default to no blending, write to color directly for all components.
    ...
    // connect the renderInfo to the pNext extension mechanism (dynamic rendering)
