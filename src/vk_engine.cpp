...
void VulkanEngine::draw_geometry(VkCommandBuffer cmd, const FrameData& frame)
{
    ...
    *sceneUniformData = sceneData;
    ...
    auto draw = [&](const RenderObject& draw) {
        ...
        // two descriptor sets per pipeline.
...
void GLTFMetallic_Roughness::build_pipelines(VulkanEngine* engine)
{
    ...
    if (!vkutil::load_shader_module(meshFragPath.c_str(), engine->_device, &meshFragShader)) {
        fmt::println("Error when building the triangle fragment shader module");
    }

    VkShaderModule meshVertexShader;
    if (!vkutil::load_shader_module(meshVertPath.c_str(), engine->_device, &meshVertexShader)) {
    ...
    //render format
    pipelineBuilder.set_color_attachment_format(engine->_frames[0]._drawImage.imageFormat);
    pipelineBuilder.set_depth_format(engine->_frames[0]._depthImage.imageFormat);
    // use the triangle layout we created
